...
/** Cache key independent of element ids: provider + languages + text. */
export function buildCacheKey(
  ...
  return hashText(`${providerId}|${sourceLanguage}|${targetLanguage}|${text}`);
}
...
export async function cacheGet(texts: {
  key: string;
  text: string;
}[], providerId: string, sourceLanguage: string, targetLanguage: string): Promise<Map<string, string>> {
  const hits = new Map<string, string>();
  if (texts.length === 0) return hits;
  const storageKeys = texts.map(({ key, text }) => ({
    storageKey: CACHE_KEY_PREFIX + buildCacheKey(providerId, sourceLanguage, targetLanguage, text),
    key,
  }));
...
export async function cachePut(
  ...
  for (const { text, translated } of items) {
    const hash = buildCacheKey(providerId, sourceLanguage, targetLanguage, text);
